/*
 * File: KarelSection.js
 * ---------------------
 * This file combines all the demos.
 */
...
function ClimbFixedSizeMountainDemo() {
    new ClimbFixedSizeMountainTrace();
}

class ClimbFixedSizeMountainTrace extends KarelTrace {

    constructor(title="ClimbFixedSizeMountain") {
        super(title);
        this.install();
    }
    ...
    setParameters() {
        this.setMaxStackDepth(2);
        this.setFrameHeight(ClimbFixedSizeMountainTrace.FRAME_HEIGHT);
        this.setFrameDeltas(ClimbFixedSizeMountainTrace.FRAME_DX,
                            ClimbFixedSizeMountainTrace.FRAME_DY);
        this.keepLastFrame(true);
    }

    defineFunctions() {
        this.defineFunction("climb_mountain", new ClimbFixedSizeMountain());
...
ClimbFixedSizeMountainTrace.FRAME_HEIGHT = 530;
ClimbFixedSizeMountainTrace.FRAME_DX = 16;
ClimbFixedSizeMountainTrace.FRAME_DY = 62;

class ClimbFixedSizeMountain extends CTFunction {

    constructor() {
        super(ClimbFixedSizeMountain.HTML);
    }
    ...
    async run(ct) {
        ...
        await ct.traceStep("#2");
        for (let i = 0; i < 4; i++) {
	    await ct.traceStep("#2a");
	    await ct.traceStep("#3", async function() {
                                         await ct.call("step_up");
                                     });
        }
        await ct.traceStep("#4", ct.putBeeper);
        await ct.traceStep("#5");
        for (let i = 0; i < 4; i++) {
	    await ct.traceStep("#5a");
	    await ct.traceStep("#6", async function() {
                                         await ct.call("step_down");
                                     });
        }
...
ClimbFixedSizeMountain.HTML =
    ...
    "    <span class='#5'><span class='keyword'>for</span> " +
         "<span class='#5a'>i <span class='keyword'>in</span> " +
         "<span class='builtin'>range</span>(4)</span>:</span>\n" +
    "        <span class='#6'>step<span class='u'>_</span>down()</span>\n" +
    "    <span class='#7'>move<span class='u'>_</span>to" +
...
StepDown.HTML =
    "<span class='keyword'>def</span> step<span class='u'>_</span>up():\n" +
    "    <span class='#1'>move()</span>\n" +
    "    <span class='#2'>turn<span class='u'>_</span>right()</span>\n" +
    "    <span class='#3'>move()</span>\n" +
    "    <span class='#4'>turn<span class='u'>_</span>left()</span>\n";
...
function ClimbMolehillDemo() {
    new ClimbMolehillTrace();
}

class ClimbMolehillTrace extends ClimbMountainTrace {
    ...
    init() {
        let world = new KarelWorld(9, 4);
        let karel = world.getKarel();
        world.setWall(4, 1, Direction.WEST);
        world.setWall(4, 2, Direction.SOUTH);
        world.setWall(5, 2, Direction.WEST);
        world.setWall(5, 3, Direction.SOUTH);
        world.setWall(6, 2, Direction.WEST);
        world.setWall(6, 2, Direction.SOUTH);
        world.setWall(7, 1, Direction.WEST);
        karel.setBeepersInBag(1);
        this._world = world;
        this._karel = karel;
    }
